"""Exercise normalized ESPN free agents against a known public historical league.

Run with ``uv run python scripts/public_free_agents_smoke_test.py`` from tools/espn-mcp.
This intentionally builds a credential-free configuration and never reads .env.
"""

from __future__ import annotations

import asyncio
import json

from espn_mcp.client import EspnConfig
from espn_mcp.server import free_agents

PUBLIC_SEASON = 2020
PUBLIC_LEAGUE_ID = "899513"


async def main() -> int:
    result = json.loads(
        await free_agents(
            EspnConfig(
                season=PUBLIC_SEASON,
                league_id=PUBLIC_LEAGUE_ID,
                espn_s2=None,
                swid=None,
            ),
            PUBLIC_LEAGUE_ID,
            limit=3,
            season=PUBLIC_SEASON,
        )
    )

    assert result["schema_version"] == 1
    assert result["season"] == PUBLIC_SEASON
    assert result["league_id"] == PUBLIC_LEAGUE_ID
    assert result["returned"] == 3
    assert all(player["player_id"] for player in result["players"])
    assert all(player["status"] in {"FREEAGENT", "WAIVERS"} for player in result["players"])
    assert all(player["is_available"] is True for player in result["players"])
    print(json.dumps(result, indent=2))
    return 0


if __name__ == "__main__":
    raise SystemExit(asyncio.run(main()))

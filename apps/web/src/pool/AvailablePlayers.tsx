...
export function AvailablePlayers() {
  const { players, adjustments, picks, adjustPlayer, settings, draftPlayer } = useDraftStore()
  ...
  const available = useMemo(() => {
    return players
      .filter((player) => !drafted.has(player.id) && (filter === 'ALL' || player.position === filter) && player.name.toLowerCase().includes(search.toLowerCase()))
      .sort((a, b) => compareAvailablePlayers(a, b, adpSource, adpDir, adjustments))
  }, [players, drafted, filter, search, adpSource, adpDir, adjustments])
